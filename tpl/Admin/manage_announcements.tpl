...
{foreach from=$announcements item=announcement}
	...
	<tr class="{$rowCss}">
		...
		<td align="center">
			...
			<a href="#" class="update email">{translate key='SendAsEmail'}</a> |
...
<div id="emailDialog" class="dialog" style="display:none;" title="{translate key=SendAsEmail}">
	<form id="emailForm" method="post">
		<div>
			<label for="groupToEmail">{translate key=Groups}</label>
			<select id="groupToEmail" {formname key=GROUP_ID}>
				<option value="">{translate key=AllUsers}</option>
		...
		<br/><br/>
		<button type="button" class="button save">{html_image src="mail-send.png"} {translate key='Email'}</button>
		<button type="button" class="button cancel">{html_image src="slash.png"} {translate key='Cancel'}</button>
	</form>
</div>
...
<script type="text/javascript">
	...
	var actions = {
		...
		sendEmail: '{ManageAnnouncementsActions::SendEmail}'
	};
